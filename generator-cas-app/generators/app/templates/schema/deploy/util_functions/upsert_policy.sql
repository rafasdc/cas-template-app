-- Deploy <%- projectName %>:database_functions/upsert_policy to pg
...
-- example: select <%- schemaName %>_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'true');
-- becomes: create policy test_policy on <%- schemaName %>.test_table for update to admin using(true) with check(true);
create or replace function <%- schemaName %>_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
  role_name text,
  using_check_statement text,
...
$function$
  ...
  begin
      ...
      if (select true from pg_policies where policyname = policy_name and tablename = table_name and schemaname = table_schema_name) then
        common_execute_string := ('alter policy ' || quote_ident(policy_name) ||
                        ' on ' || quote_ident(table_schema_name) || '.' || quote_ident(table_name) ||
                        ' to ' || quote_ident(role_name));
      else
        common_execute_string := ('create policy ' || quote_ident(policy_name) ||
                        ' on ' || quote_ident(table_schema_name) || '.' || quote_ident(table_name) || ' for ' || operation ||
                        ' to ' || quote_ident(role_name));
      end if;
...
-- example: select <%- schemaName %>_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'using(true)', 'with check(false)');
-- becomes: create policy test_policy on <%- schemaName %>.test_table for update to admin using(true) with check(false);
create or replace function <%- schemaName %>_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
...
comment on function <%- schemaName %>_private.upsert_policy(text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies';
comment on function <%- schemaName %>_private.upsert_policy(text, text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies for an update operation when the using statement and with check statement differs';
